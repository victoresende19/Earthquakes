...
projeto = st.sidebar.selectbox('Projeto', ('Mapa', 'Previsão magnitude', 'Documentação'))
...
# Pagina mapas
    ...
    startTime = form.date_input("Data inicial (ano/mês/dia):", datetime.datetime.now(pytz.timezone('America/Sao_Paulo')))
...
# Pagina previsao
    ...
    with col2:
        ...
        latitude = form1.slider('Latitude: ', min_value=-90.0, max_value=180.0, value=90.0)
        longitude = form1.slider('Longitude: ', min_value=-180.0, max_value=180.0, value=142.0)
        profundidade = form1.slider('Profundidade: ', min_value=5.0, max_value=500.0, value=15.0)
    ...
    st.markdown(f"<h4 style='text-align: left; color: black;'>Caso o terremoto ocorresse em uma latitude de {latitude}, longitude de {longitude} e o epicentro estiver a uma profundidade de {profundidade} km, a magnitude estimada desse tremor seria: </h4>", unsafe_allow_html=True)
